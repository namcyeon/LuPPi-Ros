...
namespace Yato
{
    public class Direct2DRenderer : IDisposable
    {
        ...
        public void DrawRectangle2(float x, float y, float z, float e, float stroke, Direct2DColor color)
        {
            sharedBrush.Color = color;
            device.DrawRectangle(new RawRectangleF(x, y, z, e), sharedBrush, stroke);
        }
        ...
        public void DrawText2(string text, float x, float y, Direct2DFont font, Direct2DBrush brush, Direct2DBrush blackbrush)
        {
            device.DrawText(text, text.Length, font, new RawRectangleF(x + 1, y, float.MaxValue, float.MaxValue), blackbrush, DrawTextOptions.NoSnap, MeasuringMode.Natural);
            device.DrawText(text, text.Length, font, new RawRectangleF(x - 1, y, float.MaxValue, float.MaxValue), blackbrush, DrawTextOptions.NoSnap, MeasuringMode.Natural);
            device.DrawText(text, text.Length, font, new RawRectangleF(x, y + 1, float.MaxValue, float.MaxValue), blackbrush, DrawTextOptions.NoSnap, MeasuringMode.Natural);
            device.DrawText(text, text.Length, font, new RawRectangleF(x, y - 1, float.MaxValue, float.MaxValue), blackbrush, DrawTextOptions.NoSnap, MeasuringMode.Natural);
            device.DrawText(text, text.Length, font, new RawRectangleF(x, y, float.MaxValue, float.MaxValue), brush, DrawTextOptions.NoSnap, MeasuringMode.Natural);
        }
